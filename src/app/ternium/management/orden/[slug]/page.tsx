'use client';

import { useRouter, useParams } from 'next/navigation';
import { FiArrowLeft, FiCheckCircle, FiXCircle } from 'react-icons/fi';
import LoadingSpinner from '@/components/LoadingSpinner';
import { useManagementDetail } from '@/hooks/management/useManagementDetail';
import { useRoleGuard } from '@/hooks/useRoleGuard';

export default function ManagementDetailPage() {
  useRoleGuard('/ternium/management');
  const router = useRouter();
  const params = useParams();
  const orderId = params.slug as string;

  const { order, loading, saving, error, approveOrder, rejectOrder } =
    useManagementDetail(orderId);

  if (loading) {
    return (
      <div className="min-h-screen bg-[#f8fafc] p-6 lg:p-10">
        <LoadingSpinner size="large" message="Cargando detalle..." fullScreen />
      </div>
    );
  }

  if (error || !order) {
    return (
      <div className="min-h-screen bg-[#f8fafc] p-6 lg:p-10">
        <div className="max-w-5xl mx-auto">
          <div className="bg-red-50 border border-red-200 rounded-lg p-6">
            <p className="text-red-600 text-sm">{error || 'No se encontró la orden'}</p>
            <button
              onClick={() => router.push('/ternium/management')}
              className="mt-4 px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-bold hover:bg-red-700 transition-colors"
            >
              Volver
            </button>
          </div>
        </div>
      </div>
    );
  }

  const dvStatus = order.dispatch_validation?.status ?? null;
  const isPending = dvStatus === 'Pendiente';
  const specs = order.specs;
  const ed = order.execution_details;

  const comparisonRows = [
    {
      campo: 'Peso Mín. (ton)',
      especificado: specs?.minimum_shipping_weight != null ? `${specs.minimum_shipping_weight} ton` : '—',
      ejecutado: ed?.weight != null ? `${ed.weight} ton` : '—',
    },
    {
      campo: 'Peso Máx. (ton)',
      especificado: specs?.maximum_shipping_weight != null ? `${specs.maximum_shipping_weight} ton` : '—',
      ejecutado: '—',
    },
    {
      campo: 'Embalaje',
      especificado: specs?.shipping_packaging || '—',
      ejecutado: ed?.shipping_packaging || '—',
    },
    {
      campo: 'Nota de ejecución',
      especificado: '—',
      ejecutado: ed?.note || '—',
    },
  ];

  return (
    <div className="min-h-screen bg-[#f8fafc] p-6 lg:p-10 font-sans text-slate-700">
      <div className="max-w-5xl mx-auto">

        {/* Breadcrumb */}
        <button
          onClick={() => router.push('/ternium/management')}
          className="flex items-center gap-2 text-slate-500 hover:text-slate-800 mb-6 transition-colors text-sm"
        >
          <FiArrowLeft className="w-4 h-4" />
          Volver a Order Management
        </button>

        {/* Header card */}
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-6">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">
                Detalle de Validación
              </p>
              <h1 className="text-2xl font-extrabold text-[#1e293b] tracking-tight">
                ORD-{order.id}
              </h1>
              <p className="text-slate-500 text-sm mt-1">
                {order.client?.name} · {order.product?.pt}
              </p>
            </div>
            <DispatchStatusBadge status={dvStatus} />
          </div>

          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-6 pt-6 border-t border-slate-100">
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Producto (PT)</p>
              <p className="font-semibold text-slate-800 text-sm">{order.product?.pt || '—'}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Master</p>
              <p className="font-semibold text-slate-800 text-sm">{order.product?.master || '—'}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Cliente</p>
              <p className="font-semibold text-slate-800 text-sm">{order.client?.name || '—'}</p>
            </div>
          </div>
        </div>

        {/* Main content: two columns */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">

          {/* Left: Comparación técnica */}
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-100">
              <h2 className="font-bold text-slate-800 text-sm uppercase tracking-wider">
                Comparación Técnica
              </h2>
            </div>
            <table className="w-full text-left">
              <thead className="bg-[#fcfdfe] border-b border-slate-100">
                <tr className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">
                  <th className="px-4 py-3">CAMPO</th>
                  <th className="px-4 py-3">ESPECIFICADO</th>
                  <th className="px-4 py-3">EJECUTADO</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {comparisonRows.map((row) => (
                  <tr key={row.campo} className="text-sm">
                    <td className="px-4 py-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                      {row.campo}
                    </td>
                    <td className="px-4 py-3 font-medium text-slate-700">{row.especificado}</td>
                    <td className="px-4 py-3 font-medium text-slate-700">{row.ejecutado}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>

          {/* Right: Reglas de despacho */}
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <h2 className="font-bold text-slate-800 text-sm uppercase tracking-wider mb-5">
              Reglas de Despacho
            </h2>
            <div className="flex flex-col gap-3">
              <SpecCard label="Peso Mínimo (ton)" value={specs?.minimum_shipping_weight} unit="ton" />
              <SpecCard label="Peso Máximo (ton)" value={specs?.maximum_shipping_weight} unit="ton" />
              <SpecCard label="Piezas por Paquete" value={specs?.pieces_per_package} unit="pzs" />
              <SpecCard label="Embalaje Requerido" value={specs?.shipping_packaging} />
              <SpecCard label="Ancho Máx. Tarima" value={specs?.maximum_pallet_width} unit="mm" />
            </div>
          </div>
        </div>

        {/* Decisión Final */}
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <h2 className="font-bold text-slate-800 text-sm uppercase tracking-wider mb-6">
            Decisión Final
          </h2>

          {isPending ? (
            <div className="flex flex-col sm:flex-row gap-4 justify-end">
              <button
                onClick={rejectOrder}
                disabled={saving}
                className="px-6 py-3 border border-red-400 text-red-600 hover:bg-red-50 font-bold rounded-xl transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {saving ? 'Procesando...' : 'Rechazar Validación'}
              </button>
              <button
                onClick={approveOrder}
                disabled={saving}
                className="px-8 py-3 bg-[#ff4301] hover:bg-[#e63d01] text-white font-bold rounded-xl transition-all shadow-lg hover:-translate-y-0.5 active:scale-95 text-sm uppercase tracking-wider disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0"
              >
                {saving ? 'Procesando...' : 'Aprobar para Despacho'}
              </button>
            </div>
          ) : (
            <div className="flex items-center gap-3">
              {dvStatus === 'Aceptado' ? (
                <>
                  <FiCheckCircle className="text-green-600" size={20} />
                  <span className="text-green-700 font-semibold text-sm">
                    Orden aprobada para despacho
                    {order.dispatch_validation?.approved_at && (
                      <span className="text-slate-400 font-normal ml-2 text-xs">
                        · {new Date(order.dispatch_validation.approved_at).toLocaleDateString('es-MX', {
                          day: '2-digit', month: 'short', year: 'numeric',
                          hour: '2-digit', minute: '2-digit',
                        })}
                      </span>
                    )}
                  </span>
                </>
              ) : (
                <>
                  <FiXCircle className="text-red-500" size={20} />
                  <span className="text-red-600 font-semibold text-sm">
                    Orden rechazada
                  </span>
                </>
              )}
            </div>
          )}
        </div>

      </div>
    </div>
  );
}

function DispatchStatusBadge({ status }: { status: string | null }) {
  const map: Record<string, { label: string; classes: string }> = {
    Pendiente: { label: 'Pendiente', classes: 'bg-yellow-100 text-yellow-700 border-yellow-200' },
    Aceptado:  { label: 'Aceptado',  classes: 'bg-green-100 text-green-700 border-green-200' },
    Rechazado: { label: 'Rechazado', classes: 'bg-red-100 text-red-700 border-red-200' },
  };

  const cfg = status ? (map[status] ?? map['Pendiente']) : map['Pendiente'];

  return (
    <span className={`inline-flex px-3 py-1.5 text-xs font-bold rounded-lg border ${cfg.classes}`}>
      {cfg.label}
    </span>
  );
}

function SpecCard({ label, value, unit }: { label: string; value: string | number | null | undefined; unit?: string }) {
  const display = value != null && value !== '' ? `${value}${unit ? ` ${unit}` : ''}` : '—';
  return (
    <div className="flex justify-between items-center p-3 rounded-lg bg-[#fbfbfc] border border-slate-50">
      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{label}</span>
      <span className="text-sm font-bold text-slate-800">{display}</span>
    </div>
  );
}
